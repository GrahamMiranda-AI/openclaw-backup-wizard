...
router.get('/login', (req, res) => {
  ...
  res.send(renderPage({ title: `${APP_NAME} - Login`, body, logoPath: LOGO_PATH, appName: APP_NAME }));
});
...
router.post('/login', async (req, res) => {
  ...
  if (!ok) {
    return res.status(401).send(renderPage({ title: `${APP_NAME} - Login`, body: '<div class="card">Invalid login.</div>', logoPath: LOGO_PATH, appName: APP_NAME }));
  }
...
router.get('/wizard', isAuthed, async (req, res) => {
  ...
  res.send(renderPage({ title: `${APP_NAME} - Wizard`, body, status: req.query.ok ? decodeURIComponent(req.query.ok) : '', logoPath: LOGO_PATH, appName: APP_NAME }));
});
...
router.post('/backup', isAuthed, async (req, res) => {
  try {
    ...
    res.status(500).send(renderPage({ title: `${APP_NAME} - Error`, body: `<pre>${String(err.stack || err)}</pre>`, logoPath: LOGO_PATH, appName: APP_NAME }));
  }
});
...
router.post('/delete-backup', isAuthed, async (req, res) => {
  try {
    ...
    return res.status(500).send(renderPage({ title: `${APP_NAME} - Delete Error`, body: `<pre>${String(err.stack || err)}</pre>`, logoPath: LOGO_PATH, appName: APP_NAME }));
  }
});
...
router.post('/restore', isAuthed, upload.single('backup'), async (req, res) => {
  try {
    ...
    res.status(500).send(renderPage({ title: `${APP_NAME} - Restore Error`, body: `<pre>${String(err.stack || err)}</pre>`, logoPath: LOGO_PATH, appName: APP_NAME }));
  }
});
